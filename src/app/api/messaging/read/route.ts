...
import { markConversationRead } from '@/lib/messaging/handlers'
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await markConversationRead(auth.uid, body.conversationId.trim())
    return NextResponse.json(result)
  } catch (error) {
    const { message, status } = apiErrorResponse(error)
    console.error('[messaging/read]', auth.uid, message)
