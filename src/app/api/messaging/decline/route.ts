...
import { declineConversation } from '@/lib/messaging/handlers'
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await declineConversation(auth.uid, body.conversationId.trim())
    return NextResponse.json(result)
  } catch (error) {
    const { message, status, code, retryAfterSeconds } = apiErrorResponse(error)
    console.error('[messaging/decline]', auth.uid, message)
